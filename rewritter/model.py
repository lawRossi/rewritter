...
class DecoderLayer(nn.Module):
    ...
    def forward(self, q, k, v, src_mask, tgt_mask, segment_type):
        q_ = self.norm1(q)
        m = q + self.dropout(self.mha1(q_, q_, q_, tgt_mask))
        m_ = self.norm2(m)
        mask_h = src_mask & (segment_type == 0).unsqueeze(1)
        c_h = m + self.dropout(self.mha_h(m_, k, v, mask_h))
        c_h = self.norm3(c_h)
        mask_u = src_mask & (segment_type == 1).unsqueeze(1)
        c_u = m + self.dropout(self.mha_u(m_, k, v, mask_u))
        c_u = self.norm4(c_u)
